...
void write1616GDRAM(unsigned char x,unsigned char y,unsigned char sign,unsigned char *bmp)	
{ 
	unsigned char i,j,basex;
	WriteLCM(0,0x36);      
	if(y==1||y==2)	  
	{
		basex=0x80;	   
...
char i=0,Start_pos;
	...
	if(dis_pos!=0)
		WriteLCM (0,(Start_pos+dis_pos-1));
    while(chn[i]!='\0')
    {
	   	WriteLCM (1,chn[i]);
	    i++;
...
void photodisplay(const unsigned char *bmp)	
{ 
	  unsigned char i,j;
	  WriteLCM(0,0x36);      
	  for(i=0;i<32;i++)

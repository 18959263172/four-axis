...
void spi_init()
{
	...
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->DIVIDER.DIVIDER=5;

	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->SSR.AUTOSS=0;
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->SSR.SS_LTRIG=1;
	
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->SSR.SSR=0;
	
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->CNTRL.CLKP=0;  
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->CNTRL.TX_BIT_LEN=0x08;  
}
unsigned char spi_enable()
{
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->SSR.SSR=1;
}
unsigned char spi_disable()
{
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->SSR.SSR=0;
}
...
{
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->TX[0]=data;
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->CNTRL.GO_BUSY=0x01;
	while(((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->CNTRL.GO_BUSY==1) ;
	
	
	
	return ((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->RX[0];
}
...
{
	unsigned char ret;
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->TX[0]=0xff;
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->CNTRL.GO_BUSY=0x01;
	while(((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->CNTRL.GO_BUSY==1) ;
	ret=((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->RX[0];
	((SPI_T *) ((( uint32_t)0x40000000) + 0x30000))->CNTRL.IF=1;
	return ret;
}

unsigned char nrf_read(unsigned char adress)
{
	unsigned char ret;
	spi_enable();
	spi_send(adress);
...
unsigned char nrf_write(unsigned char adress,unsigned char data)
{
	unsigned char ret;
	spi_enable();
	spi_send(adress);
...
unsigned char nrf_writebuf(unsigned char reg, unsigned char *pBuf, unsigned char len)
{
	unsigned char status,u8_ctr;	    
	spi_enable();
  	status = spi_send(reg);
  	for(u8_ctr=0; u8_ctr<len; u8_ctr++)spi_send(*pBuf++); 
		spi_disable();  	
	return status;          
}				

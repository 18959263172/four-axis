...
{     
	...
	nrf_writebuf(0x20+0x10,(unsigned char*)TxAddr,5);
	nrf_writebuf(0x20+0x0a,(unsigned char*)RxAddr,5); 
	
	nrf_write(0x20 + 0x01, 0x01); 
	nrf_write(0x20 + 0x02, 0x01); 
	nrf_write(0x20 + 0x05, 40); 
	nrf_write(0x20 + 0x11, 4);
	nrf_write(0x20 + 0x06, 0x0f);
	nrf_write(0x20 + 0x00, 0x0f); 
...
void NRFSetTxMode(unsigned char *TxDate)
{
  	...
  	nrf_writebuf(0x20+0x10,(unsigned char*)TxAddr,5);
  	nrf_writebuf(0x20+0x0a,(unsigned char*)TxAddr,5); 
		ret=nrf_read(0x07);
		delay_s();
  	nrf_write(0x20+0x07,0x10);     
		ret=nrf_read(0x07);
	
  	nrf_write(0x20+0x01,0x01);     
  	nrf_write(0x20+0x02,0x01); 
  	nrf_write(0x20+0x04,0x1a);
  	nrf_write(0x20+0x05,40);       
  	nrf_write(0x20+0x06,0x0f);  
  	nrf_write(0x20+0x00,0x0e);    
		DrvGPIO_SetBit(E_GPB,1);	
}		  
...
unsigned char NRF24L01_TxPacket(unsigned char *txbuf)
{
	unsigned char sta;
	DrvGPIO_ClrBit(E_GPB, 1);	
  nrf_writebuf(0xa0,txbuf,4);
 	DrvGPIO_SetBit(E_GPB,1);	
	while(DrvGPIO_GetBit(E_GPB, 2)!=0);
	sta=nrf_read(0x07);  
	nrf_write(0x20+0x07,sta); 
	if(sta&0x10)
	{
		nrf_wf(0xe1);
			nrf_write(0x20+0x07,sta); 
...
unsigned char NRF24L01_RxPacket(unsigned char *rxbuf)
{
	...
	if(sta&0x40)
	{
		NRF24L01_Read_Buf(0x61,rxbuf,4);
		nrf_write(0xe2,0xff);
		return 0; 
	}	   
	return 1;
